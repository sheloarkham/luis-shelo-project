...
const Presupuestos = () => {
  ...
  const totalGeneral = categorias.reduce((sum, cat) => sum + calcularTotal(cat), 0)
  const totalItems = categorias.reduce((sum, cat) => sum + gastos[cat].length, 0)
      ...
      <div className="content neo-page__content">
        <main className="presupuestos-main">
          <PageHero
            ...
            stats={[
              { value: formatearMonto(totalGeneral), label: 'Total general' },
              { value: gastos[categoriaActual].length, label: `Gastos · ${categoriaNombres[activeTab]}` },
              { value: formatearMonto(totalCategoria), label: 'Total categoría' },
            ]}
          />
          ...
          <footer className="neo-footer presupuestos-footer">
            <span className="neo-footer__pulse" aria-hidden="true" />
            <p>{totalItems} gastos registrados · {categorias.length} categorías</p>
          </footer>
        </main>
      </div>
    </div>
